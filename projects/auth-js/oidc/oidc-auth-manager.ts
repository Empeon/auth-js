...
export class OIDCAuthManager extends AuthManager<OIDCAuthSettings> {
        ...
        if (args?.refreshToken) {
            await this.#userManager?.storeUser(
                new User({
                    ...
                    access_token: this.#user?.access_token ?? undefined as unknown as string,
                    profile: this.#user?.profile ?? undefined as unknown as UserProfile,
                    token_type: this.#user?.token_type ?? undefined as unknown as string
                })
            );
        }
    ...
    // --- HANDLER(s) ---
    public onUserChanged(handler: AuthSubscriber<[User | null | undefined]>): AuthSubscription {
        return this.#userSubs.add(handler);
    }
